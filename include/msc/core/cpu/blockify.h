#ifndef ___BLOCKIFY___
#define ___BLOCKIFY___
	...
	for (ulong bc = 0; bc < matrix_block_col_count; ++bc) {
		for (ulong br = 0; br < matrix_block_row_count; ++br) {
			...
			for (ulong c = 0; c < block_col_count; ++c) {
				      T * const destiny = blocks + block_first + c * block_row_count;
				const T * const source = matrix + matrix_block_col_first + c * matrix_row_count + block_row_limits[0];
