...
void parse_arguments (int argc, char *argv[]) {
	try {
		...
		visible_options.add_options()
			...
			(
				"input-filename,f",
				value<string>(&input_filename)->default_value(""),
				"Name of the file to load the matrix (defaults to standard input)."
			)
			(
				"output-filename,o",
				value<string>(&output_filename)->default_value(""),
				"Name of the file to save the matrix (defaults to standard output)."
			)
		;

		options_description command_line_options;
		command_line_options.add(visible_options);
